...
**Summary:**
- Tables in scope: 41
- Partitioning recommended: 13 (12 LIST+RANGE, 1 HASH)
...
|---|---:|---:|---|---|---|---:|---|
| `d365.generaljournalaccountentry` | 475022861 | 431676.92 | `modifieddatetime` | LIST+RANGE | LIST(DataAreaId) + monthly | 1 | 475M rows / 431 GB; modifieddatetime 2020-10 to 2026-06; biggest GL table; composite LIST(dataareaid) → RANGE(date) |
| `d365.salesline` | 75651260 | 187890.47 | `shippingdaterequested` | LIST+RANGE | LIST(DataAreaId) + monthly | 3 | 75M rows / 187 GB; shippingdaterequested 2019-2026 clean (use this over shippingdateconfirmed which has 1900-01-01 entries); 3 DataAreaIds |
| `d365.salestable` | 59872716 | 110389.58 | `deliverydate` | LIST+RANGE | LIST(DataAreaId) + monthly | 3 | 59M rows / 110 GB; deliverydate 2019-2026; sales order header - partner of salesline |
| `d365.generaljournalentry` | 153010212 | 109012.57 | `accountingdate` | LIST+RANGE | LIST(DataAreaId) + monthly | 1 | 153M rows / 109 GB; accountingdate 2016-2026 - 10-year span; finance API filters by date AND legal entity |
| `d365.usvsalescommissionresptable` | 48530978 | 28231.41 | `invoicedate` | LIST+RANGE | LIST(DataAreaId) + monthly | 1 | 48M rows / 28 GB; invoicedate 2023-2026; composite LIST(dataareaid) → RANGE(date) |
| `d365.ledgerjournaltable` | 27696168 | 24628.89 | `createddatetime` | LIST+RANGE | LIST(DataAreaId) + quarterly | 9 | 27M rows / 24 GB; createddatetime mixed (some 1900) but spans to 2026; 9 DataAreaIds; composite LIST(dataareaid) → RANGE(date) |
| `d365.whssalesline` | 34782528 | 20411.25 | `modifieddatetime` | LIST+RANGE | LIST(DataAreaId) + monthly | 1 | 34M rows / 20 GB; modifieddatetime 2023-2026; composite LIST(dataareaid) → RANGE(date) |
| `d365.subledgerjournalaccountentrydistribution` | 42583392 | 19821.84 | `createddatetime` | LIST+RANGE | LIST(DataAreaId) + monthly | 1 | 42M rows / 19 GB; createddatetime 2021-2026; composite LIST(dataareaid) → RANGE(date) |
| `d365.usvcuststatement` | 31938815 | 19514.40 | `transdate` | LIST+RANGE | LIST(DataAreaId) + monthly | 1 | 31M rows / 19 GB; transdate 2023-2026; composite LIST(dataareaid) → RANGE(date) |
| `d365.ledgertransvoucherlink` | 37065117 | 16616.34 | `transdate` | LIST+RANGE | LIST(DataAreaId) + quarterly | 12 | 37M rows / 16 GB; transdate 2017-2026; 12 DataAreaIds |
| `d365.tmssalestable` | 29892286 | 16570.37 | `modifieddatetime` | LIST+RANGE | LIST(DataAreaId) + monthly | 1 | 29M rows / 16 GB; modifieddatetime 2023-2026; composite LIST(dataareaid) → RANGE(date) |
| `d365.usvcustinvoicejourstatement` | 19966754 | 12568.24 | `invoicedate` | LIST+RANGE | LIST(DataAreaId) + monthly | 1 | 19M rows / 12 GB; invoicedate 2023-2026; composite LIST(dataareaid) → RANGE(date) |
...
| `d365.financialdimensionvalueentityfinancialtagview` | 0 | 0.00 |
| `d365.v_dirpartytable` | 0 | 0.00 |
| `d365.esker_usvopenar` | 0 | 0.00 |
| `d365.usvopenar` | 0 | 0.00 |
| `d365.usvpnccusttransdataentity` | 0 | 0.00 |
