...
foreach ($phase in @(0,2,3,4)) {
    ...
    $partitioned = $rows | Where-Object { $_.partition_type -in 'RANGE','HASH','RANGE+LIST' }
    ...
    [void]$sb.AppendLine()
    [void]$sb.AppendLine("**Source:** profiled from ``d365.*`` schema on Azure SQL ``d365a1prdsynlinkusvprod2sql01.database.windows.net / primal`` on 2026-06-04. Row counts from ``sys.dm_db_partition_stats``; date column min/max from TABLESAMPLE.")
    [void]$sb.AppendLine()
    [void]$sb.AppendLine("**Summary:**")
    [void]$sb.AppendLine("- Tables in scope: $($rows.Count)")
    [void]$sb.AppendLine("- Partitioning recommended: $($partitioned.Count) ($(@($partitioned | Where-Object {$_.partition_type -eq 'RANGE'}).Count) RANGE, $(@($partitioned | Where-Object {$_.partition_type -eq 'RANGE+LIST'}).Count) RANGE+LIST, $(@($partitioned | Where-Object {$_.partition_type -eq 'HASH'}).Count) HASH)")
